...
const getLiquidBreakdown = `-- name: GetLiquidBreakdown :one
...
type GetLiquidBreakdownRow struct {
	BankCents     int64
	CashCents     int64
	CardDebtCents int64
	VoucherCents  int64
...
func (q *Queries) GetLiquidBreakdown(ctx context.Context, userID pgtype.UUID) (GetLiquidBreakdownRow, error) {
	row := q.db.QueryRow(ctx, getLiquidBreakdown, userID)
	var i GetLiquidBreakdownRow
	err := row.Scan(
		&i.BankCents,
		&i.CashCents,
		&i.CardDebtCents,
