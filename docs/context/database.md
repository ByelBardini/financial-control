...
- **`recurring_rules`** — regras de recorrência (frequência + intervalo + início, fim opcional). Individual = transação sem regra; temporária = regra com `end_date` **ou** `max_occurrences` (mutuamente exclusivos); permanente = sem fim. **Escrita:** `POST /recurring-rules` cria a regra **E** lança a transação do período atual (kind='standard', `recurring_rule_id` setado) numa só query atômica (CTE) — "regra + lançamento de agora", sem materializar ocorrências futuras (sem agendador).
